...
export class CategoriesService {
  ...
  get(): Observable<Category[]> {
    return this._http.get<Category[]>('services/categories');
  }

  create(form: CreateCategory): Observable<Category> {
    return this._http.post<Category>('services/categories', form).pipe(
      tap(_ => this.categorisSubject.next())
    );
  }

  update(category: UpdateCategory): Observable<Category> {
    return this._http.patch<Category>(`services/categories/${category.id}`, { name: category.name }).pipe(
      tap(_ => this.categorisSubject.next())
    )
  }

  delete(id: number): Observable<any> {
    return this._http.delete(`services/categories/${id}`).pipe(
      tap(_ => this.categorisSubject.next())
    );
  }
}
